...
const AiScheduleGenerator = () => {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        alert(`Schedule saved successfully! Schedule ID: ${data.scheduleId}`);
      } else {
        const errorData = await response.json();
            ...
            {generatedSchedule ? (
              <div>
                ...
                <div className="max-h-64 overflow-y-auto space-y-2 border p-3 rounded-md">
                  {generatedSchedule.map((task, index) => (
                    <div key={index} className="bg-gray-100 p-2 rounded">
                      <p className="font-bold">{task.title} ({task.dueDate}{task.dueTime ? ` at ${task.dueTime}` : ''})</p>
                ...
                <div className="flex justify-end gap-4 mt-6">
                  ...
                  <button onClick={handleAccept} className="px-4 py-2 bg-teal-500 text-white rounded hover:bg-teal-600">Accept</button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                ...
                <div className="flex justify-end gap-4">
                  ...
                  <button type="submit" className="px-4 py-2 bg-teal-600 text-white rounded hover:bg-teal-700" disabled={isLoading}>
